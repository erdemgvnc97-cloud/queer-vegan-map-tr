...
import { MapPin, X, MessageCircle, Send, Star, Leaf } from "lucide-react";
...
const MapView = () => {
  ...
  // 📍 Sadece mekana tıklandığında yorumları çeken dinamik fonksiyon
    ...
    if (e.placeId && map) {
      ...
      service.getDetails({ placeId: e.placeId }, (place, status) => {
        if (status === "OK") {
          ...
          setSelected(p);
          fetchReviews(p.id); // Tıklanınca fetch et
        }
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${API_URL}/api/reviews/${selected.id}`, { 
        ...review, placeName: selected.name, lat: selected.lat, lng: selected.lng 
      });
  ...
  if (!isLoaded) return <div className="h-full flex items-center justify-center font-bold text-purple-400 uppercase tracking-widest">Harita Hazırlanıyor...</div>;
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      ...
      {/* Yan Panel: Airbnb Tarzı Tasarım */}
      {selected && (
        <div className="absolute top-0 right-0 w-full md:w-[420px] h-full bg-white/95 backdrop-blur-xl shadow-2xl z-20 overflow-y-auto p-8 border-l border-gray-100 animate-in slide-in-from-right text-left">
          ...
          <div className="mb-8 pt-4">
            ...
            <h3 className="text-2xl font-black text-gray-900 border-b pb-4 leading-tight uppercase tracking-tighter">{selected.name}</h3>
          </div>

          <form onSubmit={handleSubmit} className="space-y-8">
            {/* Queer Grubu */}
            <div className="bg-purple-50/50 p-6 rounded-[2rem] border border-purple-100">
              <label className="block text-xs font-black text-purple-800 mb-4 uppercase flex items-center gap-2 italic">
              ...
              <div className="flex gap-2">
                {["Evet", "Hayır"].map(o => (
                  ...
                  className={`flex-1 py-4 rounded-2xl font-black text-xs transition-all border-2 ${review.queerEmployment === o ? 'bg-purple-600 text-white border-purple-600 shadow-lg scale-105' : 'bg-white text-purple-600 border-purple-100'}`}>
            ...
            <div className="bg-green-50/50 p-6 rounded-[2rem] border border-green-100">
              <label className="block text-xs font-black text-green-800 mb-4 uppercase flex items-center gap-2 italic">
              ...
              <div className="flex gap-2">
                {["Uygun", "Orta", "Yüksek"].map(p => (
                  ...
                  className={`flex-1 py-3 rounded-xl font-black text-[10px] transition-all border-2 ${review.veganPrice === p ? 'bg-green-600 text-white border-green-600 shadow-lg scale-105' : 'bg-white text-green-700 border-green-100'}`}>
                    {p.toUpperCase()} FİYAT
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-4">
              <textarea placeholder="Deneyimlerin nasıldı?..." className="w-full p-6 rounded-[2rem] border-2 border-gray-100 bg-gray-50 focus:border-pink-300 outline-none min-h-[140px] text-sm" value={review.comment} onChange={e => setReview({...review, comment: e.target.value})} />
            ...
            <button type="submit" className="w-full py-6 bg-black text-white font-black rounded-[2rem] shadow-2xl hover:bg-gray-800 active:scale-95 transition-all text-lg flex items-center justify-center gap-3">
              YAYINLA 🚀
            </button>
          </form>

          {/* Topluluk Deneyimleri Listesi */}
          <div className="mt-16 pt-10 border-t border-gray-100">
            ...
            <div className="space-y-8">
              {selectedReviews.map((rev, i) => (
                <div key={i} className="relative pl-6 border-l-2 border-pink-100">
